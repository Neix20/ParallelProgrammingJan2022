...
//const int num_of_core = thread::hardware_concurrency();
//
...
//	}
//
//	int cur_size(-1), subset_size(-1);
//	vector<future<vector<SubsetObj>>> async_arr(num_of_core);
...
//			auto start = gSubset_arr.begin() + i * cur_size / num_of_core;
//			auto end = gSubset_arr.begin() + (i + 1) * cur_size / num_of_core;
...
//		for (int ij = 0 + tId; ij < t_gN * t_rN; ij += num_of_core) {
...
//
//UINT64 sumAll(UINT64 start, UINT64 end) {
//	UINT64 total = 0;
...
////int main() {
////
////	// Let's do a simple add all function using async-thread vs PPL Library
////	int max_num = INT32_MAX, num_of_core = thread::hardware_concurrency();
////
////	long start, stop;
////	UINT64 total;
////
...
////
////	for (UINT64 i = 0; i < num_of_core; i++) {
////		UINT64 start = i * max_num / num_of_core;
////		UINT64 end = (i + 1) * max_num / num_of_core;
////		async_arr[i] = async(sumAll, start, end - 1);
////	}
////
////	for (auto& action : async_arr) {
////		total += action.get();
...
////	start = clock();
////	total = 0;
////	vector<UINT64> total_arr(num_of_core);
////	#pragma omp parallel
////	{
////		int tId = omp_get_thread_num();
////		UINT64 tmp_total(0);
////		for (UINT64 i = 1 + tId; i <= max_num; i += num_of_core) {
////			tmp_total += i;
////		}
////		total_arr[tId] = tmp_total;
////	}
////
////	for (auto sum : total_arr) {
////		total += sum;
////	}
////
////	stop = clock();
////
////	cout << total << endl;
////	cout << "Time Taken (OMP): " << stop - start << "ms" << endl;
////
////	start = clock();
////	total = 0;
////	
////	parallel_for(int(0), max_num, [&](int i) {
////		
////	});
////	
////
